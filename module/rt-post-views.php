...
if( ! class_exists( 'RT_POST_VIEWS' )){
	...
	class RT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !TripferyTheme::$options['post_view'] || ! is_single() ){
				return;
			}
			...
			/*if( !TripferyTheme::$options['post_view'] || $page > 1  ){
				return false;
			}*/
		...
		function _posts_column_views( $defaults ){
			...
			if( TripferyTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'tripfery-core' );
			}
		...
		function _posts_custom_column_views( $column_name, $id ){
			if( $column_name === 'post_view' ){
				echo tripfery_views( '', get_the_ID() );
			}
		}
	}
...
/*----------------------------------------------------------*/
if( !function_exists( 'tripfery_views' )){

	function tripfery_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$tripfery_view_html = esc_html__( 'View' , 'tripfery-core' );
		} else {
			$tripfery_view_html = esc_html__( 'Views' , 'tripfery-core' );
		}
		
		$tripfery_views_html = $tripfery_view_html . '<span class="view-number" >' . ' ' . $view_count . '</span> ';

		return '<span class="meta-views meta-item '. $views_class .'">'. $tripfery_views_html.'</span> ';
	}
}
