...
<div class="rt-section-title has-animation <?php echo esc_attr($data['style']); ?>">
	<div class="title-holder">
		...
		<<?php echo esc_attr($data['heading_size']); ?> class="entry-title <?php echo esc_attr($data['animation']); ?> <?php echo esc_attr($data['animation_effect']); ?>" data-wow-delay="0.6s" data-wow-duration="1.4s">
			...
			<?php if ($data['line_postion'] == 'top') { ?>
				<?php if ($data['display_line']) { ?><span class="line"></span><?php } ?>
			<?php } ?>


			<?php echo wp_kses_post($data['title']); ?></<?php echo esc_attr($data['heading_size']); ?>>

		<?php if ($data['line_postion'] == 'bottom') { ?>
			<?php if ($data['display_line']) { ?><span class="line"></span><?php } ?>
		<?php } ?>
